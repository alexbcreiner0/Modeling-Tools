...
def make_rhs(params: Params) -> Callable[[float, np.ndarray], np.ndarray]:
    ...
    def rhs(t: float, y: np.ndarray) -> np.ndarray:
        ...
        # workers and capitalists are spending their money prior to any earnings acquired at start of period
        # e.g. during the previous period, workers spend their savings to make it through, THEN get their wages, THEN spend those wages over course of next period
        # capitalists spend their savings to make it through, THEN get their profits, then spend those profits over course of next period
        # e.g. previous period's profits inform the change in 
...
def simulate(params, rtol=1e-6, atol=1e-9, method="RK45") -> Dict[str, Any]:
    ...
    # rtol and atol are relative and absolute tolerances
    # dense output interpolates the points into a continuous curve that arbitrary t can be plugged into
    sol = solve_ivp(rhs, (0.0, params.T), y0, method=method, rtol=rtol, atol=atol, dense_output=True)
...
def get_trajectories(params):
    ...
    traj["epr_profit_rates"] = epr_profit_rates
    traj["epr_prices"] = epr_prices
